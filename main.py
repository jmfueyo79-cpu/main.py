...
class WebServerHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot V12-Total Activo con Filtros High Alpha (>50%)")
...
class PipelineTradingAlphaTelegram:
    def __init__(self):
        ...
        self.config = {
            'price_range': (1.50, 20.0),      # Zona de alta aceleración (penny/small-caps)
            ...
            'lockout_minutes': 15,            # Cooldown ajustado a 15 min para alta volatilidad
        ...
        self.enviar_tg("🛡️ *SISTEMA V12-HIGH ALPHA OPTIMIZADO PARA BREAKOUTS (>50%) ACTIVADO*")
    ...
    def enviar_tg(self, msg):
        try: 
            requests.post(
                ...
                json={"chat_id": "2047038250", "text": msg, "parse_mode": "Markdown"}, 
                timeout=5
            )
        except: pass
    ...
    def obtener_tickers(self):
        """
        Consigue tickers de los principales ganadores del día en Yahoo Finance
        y los combina con tu lista de seguimiento estratégica.
        """
        try:
            ...
            dynamic_tickers = [q["symbol"] for q in quotes if "^" not in q["symbol"] and len(q["symbol"]) <= 5]
        except:
            dynamic_tickers = []
        
        base_watchlist = ["FFAI", "ALLO", "CRDF", "ALT", "IOVA", "CHRS", "AVXL", "TNXP"]
        return list(set(dynamic_tickers + base_watchlist))
    ...
    def procesar(self):
        ...
        df_d = yf.download(" ".join(tickers), period="3m", interval="1d", group_by="ticker", progress=False)
        ...
        for ticker in tickers:
            try:
                ...
                rvol_diario = vol_hoy_acumulado / vol_media_10d if vol_media_10d > 0 else 0
                ...
                if es_breakout and rvol_diario >= self.config['rvol_diario_min'] and rvol_15m >= self.config['rvol_15m_min'] and (self.config['rsi_range'][0] <= rsi <= self.config['rsi_range'][1]) and ticker not in self.estado["posiciones"]:
                    
                    cat = "🔥 SÚPER COHETE CONFIRMADO (Breakout + RVOL Extremo)" if rvol_diario >= 8.0 else "⚡ BREAKOUT DE MOMENTUM ALTA PROBABILIDAD"
        ...
        for ticker in tickers:
            try:
                ...
                if rend >= 15.0 and not pos["parcial"]:
                    pos["parcial"] = True
                    self.enviar_tg(f"💰 *PARCIAL {ticker} (+15%)*. Stop movido a precio de entrada.")
                ...
                if pos["active_trailing"]:
                    ...
                    stop_loss = pos["max"] - (self.config['trailing_atr_multiplier'] * atr)
                    ...
                    if p <= stop_loss:
                        self.enviar_tg(f"🚨 *SALIDA EN {ticker}*. Rendimiento Final: `{rend:.2f}%`")
